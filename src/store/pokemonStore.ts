  ...
  actions: {
    ...
    async loadMoveDetails(pokemon: Pokemon): Promise<void> {
      ...
      try {
        ...
        pokemon.moves = pokemon.moveUrls?.map(url => ({ name: url.split('/').slice(-2)[0], power: 0, accuracy: 0, pp: 0, type: 'normal', damageClass: 'status' })) || [];
      }
    },
      ...
      try {
        ...
        const loadPokemonDetails = async (pokemon: { name: string; url: string }, delay: number): Promise<Pokemon> => {
          ...
          return {
            ...
            type: details.types.map((t: { type: { name: string } }) => t.type.name.charAt(0).toUpperCase() + t.type.name.slice(1)).join(" / "),
            image: details.sprites.front_default,
            stats: { hp: details.stats.find((s: { stat: { name: string } }) => s.stat.name === 'hp')?.base_stat || 0, attack: details.stats.find((s: { stat: { name: string } }) => s.stat.name === 'attack')?.base_stat || 0, defense: details.stats.find((s: { stat: { name: string } }) => s.stat.name === 'defense')?.base_stat || 0, speed: details.stats.find((s: { stat: { name: string } }) => s.stat.name === 'speed')?.base_stat || 0 },
        ...
        for (let i = 0; i < listData.results.length; i += batchSize) {
          ...
          const batchResults = await Promise.all(batch.map((p: { name: string; url: string }, idx: number) => loadPokemonDetails(p, idx * 100)));
    ...
    addToTeam(pokemon: Pokemon) {
      if (this.yourTeam.length >= 3) { alert("Dein Team ist bereits voll (max. 3 Pokémon)!"); return; }
      this.yourTeam.push({ ...pokemon });
    },
    addToOpponentTeam(pokemon: Pokemon) {
      if (this.opponentTeam.length >= 3) { alert("Das gegnerische Team ist bereits voll (max. 3 Pokémon)!"); return; }
      this.opponentTeam.push({ ...pokemon });
    },
    removeFromTeam(index: number) { this.yourTeam.splice(index, 1); },
    removeFromOpponent(index: number) { this.opponentTeam.splice(index, 1); },
    updateEnemyTeam(shuffled: Pokemon[]) { this.opponentTeam = shuffled; },
    updateYourTeam(shuffled: Pokemon[]) { this.yourTeam = shuffled; },

    // --- KAMPF-STEUERUNG ---
    startBattle() { this.showBattle = true; },
    handleCloseBattle() { this.showBattle = false; },
