...
const getBackSprite = computed(() =>
  yourActivePokemon.value?.image
    ? yourActivePokemon.value.image.replace(
        "/sprites/pokemon/",
...
const opponentHPPercentage = computed(
  () =>
    (opponentHealth.value[0] / (opponentActivePokemon.value?.stats?.hp || 1)) *
    100
);
...
async function handleStatusMove(move: Move, user: "player" | "opponent") {
  ...
  await displayMessage(
    `${userName} setzt ${formatMoveName(move.name)} ein!`,
    1000
  );
  ...
  switch (move.name) {
    ...
    case "schwerttanz":
      if (user === "player")
        yourStatMods.value.attack = Math.min(6, yourStatMods.value.attack + 2);
      else
...
async function loadAllPokemonMoves() {
  await Promise.all(
    [...props.yourTeam, ...props.opponentTeam].map((p) =>
      props.loadMoveDetails(p)
    )
  );
}
...
async function executeMove(move: Move) {
  ...
  if (move.accuracy && Math.random() * 100 > move.accuracy) {
    await displayMessage(
      `${yourActivePokemon.value?.name} setzt ${formatMoveName(move.name)} ein!`
    );
...
async function opponentTurn() {
  ...
  if (enemyMove.accuracy && Math.random() * 100 > enemyMove.accuracy) {
    await displayMessage(
      `Wildes ${opponentActivePokemon.value?.name} setzt ${formatMoveName(
        enemyMove.name
      )} ein!`
    );
    await displayMessage(`...aber der Angriff verfehlte!`);
  ...
  if (currentHealth.value[0] <= 0) {
    ...
    const hasViablePokemon = props.yourTeam.some(
...
<template>
  <div class="battle-screen" :class="{ 'shake-animation': screenShake }">
    <div
      class="battle-start-animation"
      :class="{ 'fade-out': !battleAnimation }"
    >
    ...
    <div class="battle-background" :class="{ 'fade-in': uiReady }">
      <div class="opponent-container">
        <div class="health-bar-container opponent-hp">
          ...
          <div class="health-text">
            {{ opponentHealth[0] || 0 }}/{{
              opponentActivePokemon?.stats?.hp || 100
            }}
          </div>
        </div>
        <img
          ...
          class="enemy-sprite"
          :class="{
            'attack-opponent-animation': opponentAttacking,
            'glow-enter': opponentEntering,
            'status-glow-animation': opponentStatusAnimating,
          }"
      ...
      <div class="player-container">
        <div class="health-bar-container player-hp">
          ...
          <div class="health-bar">
            <div
              class="health-bar-fill"
              :style="{ width: yourHPPercentage + '%' }"
            ></div>
          </div>
          <div class="health-text">
            {{ currentHealth[0] || 0 }}/{{
              yourActivePokemon?.stats?.hp || 100
            }}
          </div>
        </div>
        <img
          ...
          class="player-sprite"
          :class="{
            'attack-player-animation': playerAttacking,
            'glow-enter': playerEntering,
            'status-glow-animation': playerStatusAnimating,
          }"
          :alt="yourActivePokemon?.name"
        />
      </div>
      ...
      <transition name="fade">
        <div
          ...
          :class="[
            'damage-text',
            damageTarget === 'player' ? 'damage-player' : 'damage-opponent',
          ]"
        >
          -{{ damageAmount }}
        </div>
      ...
      <div class="battle-menu" v-if="!showFaintMessage">
          ...
          <div class="attack-list">
            ...
            >
              ...
              <span class="attack-details">
                Stärke: {{ move.power || "-" }} | Gen.:
                {{ move.accuracy || "-" }}
              </span>
            </button>
          </div>
          <button @click="activeMenu = 'main'" class="back-btn">
            ← Zurück
          </button>
        </div>
        ...
        <div v-if="activeMenu === 'pokemon'" class="pokemon-switch-menu">
          ...
          <div class="pokemon-list">
            <div
              v-for="(pokemon, index) in props.yourTeam"
              :key="pokemon.name + index"
              class="pokemon-option"
              @click="handleSwitchPokemon(index)"
              :class="{ fainted: currentHealth[index] <= 0 || index === 0 }"
            >
              <img
                :src="pokemon.image"
                class="switch-sprite"
              ...
              <div class="pokemon-info">
                ...
                <div class="small-health-bar">
                  <div
                    ...
                    :style="{
                      width:
                        (currentHealth[index] / (pokemon.stats?.hp || 1)) *
                          100 +
                        '%',
                    }"
                  ></div>
                </div>
                <span class="small-health-text"
                  >{{ currentHealth[index] || 0 }}/{{
...
.attack-category-tag.physical {
  background-color: #e74c3c;
}
.attack-category-tag.special {
  background-color: #3498db;
}
.attack-category-tag.status {
...
.item-option {
  ...
  grid-template-areas: "name count" "desc desc";
  align-items: center;
  padding: 10px 15px;
  background-color: #e8e8e8;
  border-radius: 6px;
...
.player-sprite.attack-player-animation {
...
.enemy-sprite.attack-opponent-animation {
  animation: opponentAttack 0.5s ease-out;
}
...
.enemy-sprite.glow-enter,
.player-sprite.glow-enter {
  animation: glow-enter 0.7s forwards;
}
@keyframes glow-enter {
  0% {
    opacity: 0;
    filter: brightness(0) drop-shadow(0 0 0 white);
    transform: scale(0.2);
  }
  50% {
    opacity: 1;
    filter: brightness(2.5) drop-shadow(0 0 15px white);
...
.status-glow-animation {
  animation: status-glow 0.8s ease-in-out;
}
@keyframes status-glow {
  ...
  50% {
    filter: brightness(2.5) drop-shadow(0 0 10px #a29bfe);
  }
  100% {
    filter: brightness(1);
  }
